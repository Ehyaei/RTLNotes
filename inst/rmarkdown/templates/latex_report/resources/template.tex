...
\node[font=\color{bannerTextColor},anchor=east,
  ...
  \small{\textbf{$if(banner-text)$ \Large\textbf{\rl{$banner-text$}} \\$endif$}}
  };
\end{tikzpicture}%
}
...
\newunicodechar{‌}{~}  % U+200c THIN SPACE

\usepackage{xepersian}
\settextfont[Path = src/fonts/, Scale=1.5]{XB Niloofar-Regular.ttf}
\settextfont[Path = src/fonts/, Scale=1.1]{XB Niloofar-Regular.ttf}
...
\begin{RTL}
  $if(author)$
  \huge{$for(author)$\textcolor{titlepageAuthorTextColor}{$author$}$sep$\textcolor{titlepageAuthorTextColor}{,} $endfor$}
  \vspace{0.3cm}
  $endif$
\end{RTL}
\end{flushright}
